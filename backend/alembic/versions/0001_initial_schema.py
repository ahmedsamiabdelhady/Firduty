...
def downgrade() -> None:
    ...
    postgresql.ENUM(name="week_status").drop(op.get_bind(), checkfirst=True)
    postgresql.ENUM(name="duty_type_enum").drop(op.get_bind(), checkfirst=True)
    postgresql.ENUM(name="teacher_status_enum").drop(op.get_bind(), checkfirst=True)